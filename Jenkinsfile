...
pipeline {
    ...
    environment {
        ...
        YOUR_JFROG_CREDENTIALS = credentials('YOUR_JFROG')
        YOUR_JFROG_NPMRC_FILE = credentials('YOUR_JFROG_npmrc_YOUR_REPO')
    }
    stages {
        stage('Precheck') {
            stages {
                stage('Get Environment Variables') {
                    steps {
                        script {
                             ...
                             *   https://your_org.atlassian.net/wiki/spaces/PKR/pages/212798677/Tagging+and+labeling+in+Cards+environment
                             *
                    ...
                    when { expression {env.DEPLOY} }
                        agent {
                            docker {
                                ...
                                args "-it --entrypoint="
                                registryUrl 'https://URL.YOUR_JFROG.io/'
                                registryCredentialsId "YOUR_JFROG"
                            }
                        }
                        steps {
                            script {
                    ...
                    steps {
                        dir("${env.WORKSPACE}/server") {
                          sh '''
                            ...
                            '''
                          script {
                              writeFile file: '.npmrc', text: env.YOUR_JFROG_NPMRC_FILE
                              login_to_registry()
